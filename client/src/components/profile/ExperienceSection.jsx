  ...
  return (
    <div className="card p-5 animate-slide-up shadow-xl">
      ...
      <div className="space-y-4">
        {experience.map((exp) => (
          <div key={exp._id} className="flex items-start gap-3 group">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[14px] font-semibold text-gray-800 dark:text-gray-100">{exp.title}</p>
              <p className="text-[13px] text-gray-500 dark:text-gray-400">{exp.company}{exp.location ? `, ${exp.location}` : ""}</p>
              <p className="text-[12px] text-gray-400 dark:text-gray-500 mt-0.5">
                Started: {formatDate(exp.startDate)} – Ended: {formatDate(exp.endDate)}
              </p>
              {exp.description && <p className="text-[13px] text-gray-600 dark:text-gray-300 mt-1">{exp.description}</p>}
            </div>
        ...
        {experience.length === 0 && (
          <p className="text-[13px] text-gray-400 dark:text-gray-500">No experience added yet.</p>
        )}
      </div>
